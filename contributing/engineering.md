...
```bash
git clone git@github.com:xwp/material-design-wp-theme.git material-design-google
cd material-design-google
```
...
```bash
wp theme activate material-design-google
```
...
This will create an `material-design-google.zip` in the theme directory which you can install. The contents of this ZIP are also located in the `build` directory which you can `rsync` somewhere as well if needed.
...
1. Do `npm install`.
1. Do `npm run build` and install the `material-design-google.zip` onto a normal WordPress install running a stable release build; do smoke test to ensure it works.
1. [Draft new release](https://github.com/xwp/material-design-wp-theme/releases/new) on GitHub targeting the required branch (`develop` for major, `x.y` for minor).
    ...
    1. Attach the `material-design-google.zip` build to the release.
...
1. Check out the branch intended for release (`develop` for major, `x.y` for minor) and pull latest commits.
1. Do `npm install`.
1. Bump theme versions in `material-design-google.php`. Ensure patch version number is supplied for major releases, so `1.2-RC1` should bump to `1.2.0`.
1. Ensure "Tested Up To" is updated to current WordPress version.
1. Do `npm run build` and install the `material-design-google.zip` onto a normal WordPress install running a stable release build; do smoke test to ensure it works.
1. Optionally do sanity check by comparing the `build` directory with the previously-deployed theme on WordPress.org for example: `svn export https://themes.svn.wordpress.org/material-design-google/trunk /tmp/material-design-google-trunk; diff /tmp/material-design-google-trunk/ ./build/` (instead of straight `diff`, it's best to use a GUI like `idea diff`, `phpstorm diff`, or `opendiff`).
1. [Draft new release](https://github.com/xwp/material-design-wp-theme/releases/new) on GitHub targeting the required branch (`develop` for major, `x.y` for minor):
